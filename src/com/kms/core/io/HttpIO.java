...
import com.kms.core.MainUX;
import com.kms.core.data.DataAuctionInput;
import com.kms.core.data.DataAuctionOutput;
import com.kms.core.data.DataRoot;
import com.kms.core.data.DataRoot.TYPE;
import com.kms.core.data.DataSagunInput;
import com.kms.core.data.DataSagunOutput;
import com.kms.core.parser.HtmlParserAuction;
import com.kms.core.parser.HtmlParserSagun;
...
public class HttpIO extends Thread {
    ...
    void httpInit(TYPE type)
    {
    	...
    	{
            httpclient = HttpUtil_In.getHttpClient ( SoeID, Password, 0 );
    	}
    	else
    	{
            httpclient = HttpUtil_In.getHttpClient ( SoeID, Password, 1 );
    	}
        ...
        if( !MainUX.isDevMode())
        {
            httpPost.setConfig( HttpUtil_In.getConfig());               	
        }
    }    
    ...
    boolean httpExecuteAuction(DataAuctionInput input, int tab , DataAuctionOutput output )
    {
        ...
        try {
        	    HttpUtil_In.setHttpPost1( httpPost, tab );
    ...
    boolean httpExcuteSagun(DataSagunInput input, int tab , DataSagunOutput output )
    {
        ...
        try {
            ...
            // Encoding Type 지정 
            HttpUtil_In.setHttpPost2( httpPost );
            httpPost.setEntity(new UrlEncodedFormEntity(nvps, "EUC-KR"));
